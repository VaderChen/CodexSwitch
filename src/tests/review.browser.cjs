...
test('data URL: imported ID remains data; reset/login work without localStorage; update bridge and progress work',async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{
   ...
   window.checks=0;
   window.checkForUpdate=async()=>{window.checks++;if(window.checks===1)return;window.updateState={phase:'available',message:'新版',version:'1.26.0916-build-0000'}};
  ...
  assert.equal(await page.evaluate(()=>window.checks),2,'啟動檢查與手動檢查各一次');
  await page.locator('#updateButton').click();
