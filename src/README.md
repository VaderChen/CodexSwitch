...
帳號資訊列顯示 `MM/DD HH:mm · 80% / 65%`，固定左側為五小時、右側為七天，缺少的項目顯示 `-`（例如 `- / 46%`）。滑鼠停留帳號列或以鍵盤聚焦時會顯示詳細泡泡，以三行顯示五小時／七天剩餘用量、重置時間與更新狀態。

後端使用 `https://chatgpt.com/backend-api/wham/usage` 唯讀端點，僅在後端帶入帳號憑證；剩餘比例為 `100 - used_percent`。此端點沿用本機 NR-Intern 專案的整合方式，並非 Platform API 的帳單用量；官方亦說明了 [Codex App Server 帳號用量介面](https://learn.chatgpt.com/docs/app-server)。

每個帳號快取一分鐘，最多同時三個背景請求，單次請求逾時 15 秒。介面每兩秒讀取快取，只更新用量文字，不重新排列帳號。更新失敗會保留上次成功數值，以褐色標示並於詳細泡泡說明；沒有上次數值則顯示 `-`。不自動輪替 refresh token，登入失效時可重新登入。

## 用量重置

帳號列的「重置」位於「套用」左側。點擊後背景讀取可用次數與逐筆到期時間，參照 NR-Intern 的重置流程。選定額度後，持續按住「長按重置」兩秒即確認消耗一次；滑鼠移出、放開、失焦或切換視窗都會取消，Enter／空白鍵亦需長按。

請求使用該帳號的憑證；不會自動重置或扣除次數。網路失敗後重試同一筆額度會沿用本機儲存的 `redeem_request_id`，避免重複兌換。上游未提供有效明細時，不會顯示可消耗按鈕。
